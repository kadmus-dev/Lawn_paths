...
class WalkingPathsDetector(pl.LightningModule):
    ...
    def predict(self, image, block_size):
        ...
        with torch.no_grad():
            ...
            for i in range(len(image_blocks)):
                ...
                output = self(
                    torch.from_numpy(block.transpose(2, 0, 1))[None, ...].type(torch.FloatTensor).cuda()).cpu()
    ...
    def train_dataloader(self):
        train_aug = from_dict(self._hparams["train_aug"])

        if "epoch_length" not in self._hparams["train_parameters"]:
            epoch_length = None
        else:
            epoch_length = self._hparams["train_parameters"]["epoch_length"]

        result = DataLoader(
            SegmentationDataset(self.train_samples, train_aug, epoch_length),
            batch_size=self._hparams["train_parameters"]["batch_size"],
    ...
    def val_dataloader(self):
        val_aug = from_dict(self._hparams["val_aug"])

        result = DataLoader(
            SegmentationDataset(self.val_samples, val_aug, length=None),
            batch_size=self._hparams["val_parameters"]["batch_size"],
            num_workers=self._hparams["num_workers"],
            shuffle=False,
            pin_memory=True,
            drop_last=False,
    ...
    def training_step(self, batch, batch_idx):
        ...
        preds = self.forward(features).type(torch.FloatTensor).cuda()
    ...
    def training_epoch_end(self, outputs):
        ...
        logs["train_loss"] = torch.stack([x['logs']['train_loss'] for x in outputs]).mean()
        logs["precision"] = torch.stack([x['logs']['precision'] for x in outputs]).mean()
        logs["recall"] = torch.stack([x['logs']['recall'] for x in outputs]).mean()
        logs["f1"] = torch.stack([x['logs']['f1'] for x in outputs]).mean()
    ...
    def validation_epoch_end(self, outputs):
        ...
        logs["val_loss"] = torch.stack([x['logs']['val_loss'] for x in outputs]).mean()
        logs["val_precision"] = torch.stack([x['logs']['val_precision'] for x in outputs]).mean()
        logs["val_recall"] = torch.stack([x['logs']['val_recall'] for x in outputs]).mean()
        logs["val_f1"] = torch.stack([x['logs']['val_f1'] for x in outputs]).mean()
